...
def banner_anonymous():
    try:
        art = figlet_format("Web Tools", font="slant")
    except Exception:
        art = figlet_format("WEb Tools")
    print(Fore.RED + Style.BRIGHT + art)
    print(Fore.CYAN + Style.BRIGHT + "Tools by SatyaGanzz".center(60, " "))
...
def render_md(report: Report) -> str:
    ...
    md.append("\n---\n")
    md.append("*Dibuat oleh SatyaGanzz - untuk keperluan edukasi dan pengujian legal.*\n")
    return "".join(md)
...
def main():
    ...
    if Console and Panel:
        banner = Panel.fit(
            "[bold green]WEB HACK ANALYSIS (RUNNING)\n[/bold green][bold red]Tools by SatyaGanzz[/bold red]",
            style="bold green"
        )
        Console().print(banner)

    ap = argparse.ArgumentParser(description="WEB HACK ANALYSIS - All active (deep)")
